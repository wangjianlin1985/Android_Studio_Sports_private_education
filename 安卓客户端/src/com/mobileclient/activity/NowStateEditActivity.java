...
public class NowStateEditActivity extends Activity {
	// 声明确定添加按钮
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		// 设置当前Activity界面布局
		setContentView(R.layout.nowstate_edit); 
		ImageView search = (ImageView) this.findViewById(R.id.search);
		search.setVisibility(View.GONE);
		TextView title = (TextView) this.findViewById(R.id.title);
		title.setText("编辑现状态信息");
			...
			public void onClick(View v) {
				try {
					...
					if(ET_stateName.getText().toString().equals("")) {
						Toast.makeText(NowStateEditActivity.this, "状态名称输入不能为空!", Toast.LENGTH_LONG).show();
						ET_stateName.setFocusable(true);
						ET_stateName.requestFocus();
						return;	
					}
					nowState.setStateName(ET_stateName.getText().toString());
					/*调用业务逻辑层上传现状态信息*/
					NowStateEditActivity.this.setTitle("正在更新现状态信息，稍等...");
					String result = nowStateService.UpdateNowState(nowState);
	...
	private void initViewData() {
		...
		this.ET_stateName.setText(nowState.getStateName());
	}
	@Override
