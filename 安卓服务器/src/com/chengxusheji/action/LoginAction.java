...
public class LoginAction extends ActionSupport {
	...
	public String CheckLogin() { 
		...
		if (adminDAO.CheckLogin(admin)) { 
			ctx.getSession().put("identify", "admin");
			ctx.getSession().put("username", admin.getUsername());
			return "main_view";
		} else if(managerDAO.CheckLogin(admin)) {
			ctx.getSession().put("identify", "manager");
			ctx.getSession().put("username", admin.getUsername());
			return "main_view";
		} else {
			ctx.put("error",  java.net.URLEncoder.encode("账号或密码错误"));
			return "error";
		}
