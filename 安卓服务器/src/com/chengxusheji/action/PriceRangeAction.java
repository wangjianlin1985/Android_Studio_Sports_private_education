...
public class PriceRangeAction extends BaseAction {
    ...
    public String QueryPriceRangeOutputToExcel() { 
        ...
        String[] headers = { "价格范围id","起始价","结束价","显示信息"};
        List<String[]> dataset = new ArrayList<String[]>(); 
        for(int i=0;i<priceRangeList.size();i++) {
        	PriceRange priceRange = priceRangeList.get(i); 
        	dataset.add(new String[]{priceRange.getPrId() + "",priceRange.getStartPrice() + "",priceRange.getEndPrice() + "",priceRange.getShowText()});
        }
        /*
    ...
    public String ModifyPriceRangeQuery() {
        ActionContext ctx = ActionContext.getContext();
        /*根据主键prId获取PriceRange对象*/
        PriceRange priceRange = priceRangeDAO.GetPriceRangeByPrId(prId);
    ...
    public String DeletePriceRange() {
        ActionContext ctx = ActionContext.getContext();
        try { 
            priceRangeDAO.DeletePriceRange(prId);
